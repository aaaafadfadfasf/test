...
static void DBGlobalCBProc(u16 wErrNum, LPCSTR lpSrvMsg, void *pContext)
{
    if (NULL == lpSrvMsg)
    {
        log(enLOGLVL_DEBUG, "\n\nDBGlobalCBProc ErrNum:%d, no SrvMsg\n\n", wErrNum);
        return;
    }

    log(enLOGLVL_WARN, "\n\nDBGlobalCBProc ErrNum:%d, SrvMsg:%s\n\n", wErrNum, lpSrvMsg);
}
...
bool DBIInit( ENDBType enDBType )
{
    do 
    {
        ...
        if (!SetDBLocale("", DB_CHARSET_UTF8)) // hzytodo2 linlifen说这样只是mysql版本这样写
        {
            log(enLOGLVL_ERROR, "SetDbLocale failed\n");
            break;
        }
		//if (!SetupLibErrFuc(DBGlobalCBProc, (void*)NULL))
		//{
		...
		{
			log(enLOGLVL_ERROR, "SetDBParam failed\n");
			break;
		}
