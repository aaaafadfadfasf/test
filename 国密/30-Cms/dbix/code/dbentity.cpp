...
static ENDBResult GenerateEntityAddSql(TSqlString &tSql, const CDBEntitySchema &cSchema, const map<TColName, Variant> &tColValues)
{
    ...
    map<TColName, Variant>::const_iterator it;

    const vector<TColInfo>& tColInfos = cSchema.ColumnInfos();
    for (u32 i = 0; i < tColInfos.size(); ++i)
    {
        it = tColValues.find(tColInfos[i].colName);
        if (it != tColValues.end())
        {
            tSql.AppendString("%s,", tColInfos[i].colName.c_str());
        }
    }
    tSql.EraseLastRedundantComma();
    ...
    for (u32 i = 0; i < tColInfos.size(); ++i)
    {
        ...
        if (it != tColValues.end())
        {
            tSql.AppendString("%s,", DBData(it->second, tColInfos[i].colDataType).c_str());
        }
    }
    tSql.EraseLastRedundantComma();
    tSql.AppendString(")");
                                       ...
                                       const CDBCriteria &cConditions)
{
    ...
    for (map<TColName, Variant>::const_iterator it = tColValues.begin(); it != tColValues.end(); ++it)
    {
...
ENDBResult CDBEntity::ExecImp( CDBConnection *pcDBConnection, CDbResultDataBase* &pcResultData )
{
    ...
    do 
    {
        ...
        {
            enDbResult = GenerateEntityQrySql(tSql, m_cSchema, m_cConditions, m_tOrderby, m_dwEntityIndex, m_dwEntityNum, 
                                              pcDBConnection->ConnectCfg().enDbType);
            if (enDBResultSuccess != enDbResult)
            {
                break;
            }
