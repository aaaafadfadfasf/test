...
string g_strDomainId = "ea6c07c6c6a54230ba121fc578b14b90";
string g_strDomainName = "jiangsu";
...
string GetCuiServerIpStr()
{
    ...
    std::string strCuiIp("172.16.64.74");
    return strCuiIp;
}
...
void SubscribeTvsStatus( )
{
    CCuSubscribeNotifyReq cCuSubscribeNotifyReq;
    cCuSubscribeNotifyReq.SetSession(g_strCuInCmuSess);

    CNotifySubscription cNotifySubscription;
    cNotifySubscription.SetDeviceURI("tvwallid_123456789@jiangsu");
    cNotifySubscription.SetSubscribeNotifyType(SubscribeTVWallStatus);

    cCuSubscribeNotifyReq.GetSubscribeList().push_back(cNotifySubscription);
...
//     cCuRcsGetSysParamReq.SetSession(g_strCuInCmuSess);
// 
...
void GetAlarmLinkConfig()
{
    CCuAlarmLinkCfgGetReq cCuAlarmLinkCfgGetReq;
    cCuAlarmLinkCfgGetReq.SetSession(g_strCuInCmuSess);
    TAlarmLinkSrc alarmSrc;
    alarmSrc.SetDevUri("123");
    alarmSrc.SetInputId(1);   
    alarmSrc.SetAlarmType(ALARM_TYPE_MOVE);
...
void SetAlarmLinkConfig()
{
    CCuAlarmLinkCfgSetReq cCuAlarmLinkCfgSetReq;
    cCuAlarmLinkCfgSetReq.SetSession(g_strCuInCmuSess);

    TAlarmLinkage cfg;
    cfg.SetAlarmAlias("alarmTest");
    cfg.SetIsUsed(true);

    TAlarmLinkSrc alarmSrc;
    alarmSrc.SetDevUri("55010100000000000011200011200000@china");
    alarmSrc.SetInputId(0);
    alarmSrc.SetAlarmType(ALARM_TYPE_MOVE);
    cfg.SetAlarmLinkSrc(alarmSrc);

    cfg.GetDefenceSched().push_back("2012-07-11 15:27:33");
    
    TAlarmLinkDst dst;
    dst.SetAlarmOutChn(0);
    dst.SetCamPrePos(12);
    dst.GetEncChn().SetChnNO(0);
    dst.GetEncChn().SetDevUri("55010100000000000011200011200000@china ");
    dst.SetTvDivId(0);
    dst.SetTvId(0);
    dst.SetTvWallId("tvwall@china");
   
    cfg.GetAlarmLinkDstList().push_back(dst);
...
void QueryAlarmLinkConfig()
{
    //如果不添加查询条件则为所有告警联动查询
    CCuAlarmLinkQueryReq cCuAlarmLinkQueryReq;
    cCuAlarmLinkQueryReq.SetSession(g_strCuInCmuSess);
    cCuAlarmLinkQueryReq.SetStartEntryIndex(1);
    cCuAlarmLinkQueryReq.SetExpectEntryNum(1);
...
void TVWallDelScheme()
{
    CCuTvWallSchemeDelReq cCuTvWallSchemeDelReq;
    cCuTvWallSchemeDelReq.SetSession(g_strCuInCmuSess);

    cCuTvWallSchemeDelReq.SetTvWallId("tvwallid_123456789@china");
    CTvWallSchemeNameList  schemeNameList;
    schemeNameList.push_back("s2");
...
void TVWallQueryScheme()
{
    CCuTvWallSchemeQueryReq cCuTvWallSchemeQueryReq;
    cCuTvWallSchemeQueryReq.SetSession(g_strCuInCmuSess);
    cCuTvWallSchemeQueryReq.SetTvWallId("tvwallid_123456789@china");
...
void TVWallSaveScheme()
{
    CCuTvWallSchemeSaveReq cCuTvWallSchemeSaveReq;
    cCuTvWallSchemeSaveReq.SetSession(g_strCuInCmuSess);

    TTvWallScheme scheme;
    scheme.SetSchemeName("s2");
    scheme.SetTvWallId("tvwallid_123456789@china");

    CTvWinList& tTvWinList = scheme.GetTvWinList();
    for (size_t iTv=0; iTv<4; iTv++)
    {
        TTvWin tTvWin;
        tTvWin.SetTvWinId(iTv);
        ETvWinStype eStyle = ETV_WIN_STYLE_1;
        if (iTv == 0)
        {
            eStyle = ETV_WIN_STYLE_1;
        }
        ...
        for (size_t iTvDiv=0; iTvDiv<(size_t)eStyle; iTvDiv++)
        {
            ...
            for (size_t iPollIndex=0; iPollIndex<nPollNum; iPollIndex++)
            {
                TTvPollUnit tPollUnit;
                tPollUnit.SetPollInex(iPollIndex);
                tPollUnit.GetEncChn().SetDevUri("1234567@china");
                tPollUnit.GetEncChn().SetChnNO(0);
                tPollUnit.SetPollTime(10);
    ...
    cCuTvWallSchemeSaveReq.SetScheme(scheme);
...
void TVWallLoadScheme()
{
    CCuTvWallSchemeLoadReq cCuTvWallSchemeLoadReq;
    cCuTvWallSchemeLoadReq.SetSchemeName("s2");
    cCuTvWallSchemeLoadReq.SetTvWallId("tvwallid_123456789@china");
    cCuTvWallSchemeLoadReq.SetSession(g_strCuInCmuSess);
 ...
 void TVWallStartPlay()
 {
     CCuRealMediaOnTvReq cCuRealMediaOnTvReq;
     cCuRealMediaOnTvReq.SetSession(g_strCuInCmuSess);
 
     TTvChannel tTvChn;
     tTvChn.SetTvDivId(0);
     tTvChn.SetTvId(0);
     tTvChn.SetTvWallId("tvwallid_123456789@china");
     cCuRealMediaOnTvReq.SetTvChn(tTvChn); 
 
     TChannel tEncChn;
     tEncChn.SetChnNO(0);
     tEncChn.SetDevUri("1234567@china");
 ...
 void TVWallStopPlay()
 {
     ...
     cCuRealMediaOffTvReq.SetSession(g_strCuInCmuSess);

     TChannel tDecChn;
     tDecChn.SetChnNO(0);
     tDecChn.SetDevUri("1234567@china");
 ...
 void TVWallWindowPollContrl()
 {
     CCuTvPollOperateReq cCuTvPollOperateReq;
     cCuTvPollOperateReq.SetSession(g_strCuInCmuSess);
     ...
     CTvPollList& tPollList = cCuTvPollOperateReq.GetPollList();
     for (size_t i=0; i<2; i++)
     {
         TTvPollUnit tUnit;
         tUnit.GetEncChn().SetDevUri("1234567@china");
         tUnit.GetEncChn().SetChnNO(i);
         tUnit.SetPollInex(i);
         tUnit.SetPollTime(10);
 ...
 void CallPu()
 {
     ...
     cCuStartThirdPartyCallReq.SetSession(g_strCuInCmuSess);
     ...
     //目的是PU
     TChannel tDstChn;
     tDstChn.SetDevUri("a7786f080a8a4f38bcd78bbe3d98db55@jiangsu");
     tDstChn.SetChnNO(0);
     ...
     mediaFormat.SetEncodingName("md5");
     mediaFormat.SetManufacture("kedacom");
     mediaFormat.SetPayloadType(1);
     ...
     tMediaTransChannel.SetMediaChannelKey("123456");

     TNetAddr tLocalAddr;
     tLocalAddr.SetNetIp("172.16.251.22");
     tLocalAddr.SetNetPort(2222);
     tMediaTransChannel.GetAddrList().push_back(tLocalAddr);

     TNetAddr tNetAddr;
     tNetAddr.SetNetIp("172.16.251.22");
     tNetAddr.SetNetPort(2225);
     tMediaTransChannel.GetAddrList().push_back(tNetAddr);


     tMediaDesc.GetMediaTransChnList().push_back(tMediaTransChannel);
     
     tSdp.AddMediaDesc(tMediaDesc);
 ...
 void QueryMap()
 {
     ...
     cCuMapQryReq.SetExpectEntryNum(10);
     cCuMapQryReq.SetStartEntryIndex(0);
     cCuMapQryReq.SetSession(g_strCuInCmuSess);
 ...
 void CreateMap()
 {
     ...
     cCuMapCreateReq.SetSession(g_strCuInCmuSess);

     cCuMapCreateReq.GetMapInfo().SetMapId("123456");
     cCuMapCreateReq.GetMapInfo().SetMapName("test");
     cCuMapCreateReq.GetMapInfo().SetMapType(MAP_GOOGLE);
     cCuMapCreateReq.GetMapInfo().SetMapUrl("123456@jiangsu");
     cCuMapCreateReq.GetMapInfo().SetDescription("纯属扯淡测试");

     MapPosition leftTop;
     leftTop.SetLatitude(12345);
     leftTop.SetLongitude(56789);
     MapPosition rightTop;
     rightTop.SetLatitude(56789);
     rightTop.SetLongitude(12345);
     cCuMapCreateReq.GetMapInfo().GetBounds().SetLeftTop(leftTop);
     cCuMapCreateReq.GetMapInfo().GetBounds().SetRightBottom(rightTop);

     cCuMapCreateReq.GetMapInfo().SetMaxScaleLevel(1);
     cCuMapCreateReq.GetMapInfo().SetMinScaleLevel(1);

     cCuMapCreateReq.GetMapInfo().GetCenterPos().SetLatitude(123);
     cCuMapCreateReq.GetMapInfo().GetCenterPos().SetLongitude(456);

     cCuMapCreateReq.GetMapInfo().SetDefaultScaleLevel(1);

     cCuMapCreateReq.GetMapInfo().SetVersion("123456");
 ...
 void ModifyMap()
 {
     ...
     cCuMapCreateReq.GetMapInfo().SetMapId("123456");
     cCuMapCreateReq.GetMapInfo().SetMapName("12345");
     cCuMapCreateReq.GetMapInfo().SetMapType(MAP_JPEG);
     cCuMapCreateReq.GetMapInfo().SetMapUrl("123456@jiangsu");
     cCuMapCreateReq.GetMapInfo().SetDescription("纯属扯淡测试");
 ...
 void DelMap()
 {
     ...
     cCuMapDelReq.SetSession(g_strCuInCmuSess);
     cCuMapDelReq.SetMapId("123456");
 ...
 void CreateMapElement()
 {
     ...
     cCuMapElementCreateReq.SetSession(g_strCuInCmuSess);

     cCuMapElementCreateReq.GetMapElement().SetId("123456");
     cCuMapElementCreateReq.GetMapElement().SetName("test");

     cCuMapElementCreateReq.GetMapElement().SetType(MAP_ElEMENT_COMMAN);

     cCuMapElementCreateReq.GetMapElement().SetDeviceId("123456");
     cCuMapElementCreateReq.GetMapElement().SetDeviceDomainId("123456");
     cCuMapElementCreateReq.GetMapElement().SetDeviceChannel(1);

     cCuMapElementCreateReq.GetMapElement().SetEarthCoord(true);
     
     cCuMapElementCreateReq.GetMapElement().GetPosition().SetLatitude(1234);
     cCuMapElementCreateReq.GetMapElement().GetPosition().SetLongitude(5678);

     cCuMapElementCreateReq.GetMapElement().SetDescription("test");

     CCuMapElementCreateRsp cCuMapElementCreateRsp;
     CuCreateMapElement(cCuMapElementCreateReq, cCuMapElementCreateRsp);
 }

 void ModifyMapElement()
 {
     CCuMapElementCreateReq cCuMapElementCreateReq;
     cCuMapElementCreateReq.SetSession(g_strCuInCmuSess);

     cCuMapElementCreateReq.GetMapElement().SetId("123456");
     cCuMapElementCreateReq.GetMapElement().SetName("123456");

     cCuMapElementCreateReq.GetMapElement().SetType(MAP_ELEMENT_LIVE);
     ...
     cCuMapElementCreateReq.GetMapElement().GetPosition().SetLatitude(5679);
     cCuMapElementCreateReq.GetMapElement().GetPosition().SetLongitude(1254);

     cCuMapElementCreateReq.GetMapElement().SetDescription("maptest");
 ...
 void DelMapElement()
 {
     ...
     cCuMapElementDelReq.SetSession(g_strCuInCmuSess);
     cCuMapElementDelReq.SetElementId("123456");

     CCuMapElementDelRsp cCuMapElementDelRsp;
     CuDelMapElement(cCuMapElementDelReq, cCuMapElementDelRsp);
 }

 void QueryMapElement()
 {
     CCuMapElementQryReq cCuMapElementQryReq;
     cCuMapElementQryReq.SetSession(g_strCuInCmuSess);
     cCuMapElementQryReq.SetStartEntryIndex(0);
     cCuMapElementQryReq.SetExpectEntryNum(10);

     MapElementCondition codition;
     codition.SetMapId("123456");
