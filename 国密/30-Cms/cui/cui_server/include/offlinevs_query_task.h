/*
缓存查询类业务，解决CU分页查询时，平台找不到对应TASK
比如分页查询视频源离线列表
*/
#ifndef _CACHE_QUERY_TASK_H_
#define _CACHE_QUERY_TASK_H_
...
class COfflineVsQueryTask : public CSipTask
{
public:
    enum
    {
...
public:
    COfflineVsQueryTask(CInstExt *pcInst, CCuAgent *pcCuAgent, PTRecvNode ptRecvNode);
    ~COfflineVsQueryTask();
...
public:
    ...
    {
        TASKLOG(NORMAL_TASK, EVENT_LEV, "COfflineVsQueryTask[TaskNO %u]发送消息[%s--%d]给服务器[%s]\n", 
...
public:
    ...
    u32 OnWaitRspMsg(CMessage *const pcMsg)
    {
        ...
        { //查询失败 或者 无结果
            ...
            {
                TASKLOG(NORMAL_TASK, ERROR_LEV, "COfflineVsQueryTask[NO: %u]查询应答[%s--%d]返回失败，错误码[%d]\n",
                    GetTaskNO(), cRsp.GetEventStr().c_str(), cRsp.GetEventId(), cRsp.GetErrorCode());
            }
            else
            {
                TASKLOG(NORMAL_TASK, EVENT_LEV, "COfflineVsQueryTask[NO: %u]查询应答[%s--%d]成功，结果为空\n",
	...
	// 离线统计处理函数
	void GetOfflineVSRspFromCMU(CMessage *const pcMsg);
	void SaveOfflineVSNtf(CMessage *const pcMsg);
	void ClearOffLineVSData();
	void CuQueryOffLineVSReq( CMessage *const pcMsg );
	void CuQueryOffLineVSRsp();
	void CuQueryOffLineVsWait();
public:
