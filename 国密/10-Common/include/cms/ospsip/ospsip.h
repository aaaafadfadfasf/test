...
#define UNIQUE_SIP_UA_SEC_MOD   ("security_module")
...
{
public:
	static void InitSKfHardInfo();

    static void UaRegWithCertificate(const string& uaid, bool with=true);
    static bool IsUaRegWithCertificate(const string& uaid);

	static LPCSTR LocalPrivateKey4Sign();
	static LPCSTR LocalPrivateKey4Encry();
	static int    LocalHardKeyEngineId();
	static bool   IsUsingHardKey();
    static LPCSTR LocalPrivateKeyPassword();
    static void SetLocalPrivateKeyPassword(const string&passwd);
    static string PeerPublicCert(const string& uaid);
	static string PeerPublicEncryptCert(const string& uaid);

    static void UpdateUaVkek(const string& uaid, const string&vkek);// (不包括版本号)
    static string GetUaVkek(const string& uaid);// 获取Update时保存的vkek(不包括版本号)
    static void UpdateUaAlgo(const string& uaid, const CKDSipAlgorithmTOOL&algo);
    static CKDSipAlgorithmTOOL GetUaAlgo(const string& uaid);

    static string UasBuildSecurityInfo(const string &uacid, const string &recvedAuth, const string& vkekVersion);
    static bool UacCheckSecurityInfo(const string &uasid, const string&securityInfo);

    static bool UaSecurityEncrypt(const string &uaid, const string &strIn, string &strEncrypted);
    static bool UaSecurityDecrypt(const string &uaid, const string &strIn, string &strDecrypted);
};
...
public:
    ...
    TSipURI GetSecureModuleURI() const
    {
        TSipURI tCmuNO;
        tCmuNO.m_strUser = UNIQUE_SIP_UA_SEC_MOD;
        tCmuNO.m_strDomain = m_strDomain;
...
struct TOspSipConf
{
    TOspSipConf() : tUAType(UA_TYPE_INVALID)
    {
        ...
        strSecCap = "A:SM2;H:SM3;S:SM4/OFB/PKCS5,SM4/CBC/PKCS5,SM1/OFB/PKCS5,SM1/CBC/PKCS5;SI:SM3-SM2";
    }
    ...
    u32 dwDefaultOspIID;
    s32 nUaRouteMode;
	set<string> setUnCheckSpecialMsgSafeUaList; //不校验ua特定信令安全性的ua列表
    string strSecCap;
};
...
#define  MAX_SIP_WWWAUTH_LEN    (512)
#define  MAX_SIP_SUBJECT_LEN    (1024)
...
#define  MAX_SIP_NOTE_LEN       (1024)
#define  MAX_SIP_USER_MONITOR_LEN (256)
...
public:
    ...
    void Clear()
    {
        ...
        memset(m_szSipWWWAuth, 0, sizeof(m_szSipWWWAuth));
		...
		memset(m_szNote, 0, MAX_SIP_NOTE_LEN+1);
        memset(m_szUserMonitor, 0, sizeof(m_szUserMonitor));
    }
...
public:
    ...
    char                 m_szSipWWWAuth[MAX_SIP_WWWAUTH_LEN+1];
	...
	char                 m_szNote[MAX_SIP_NOTE_LEN+1];
    char                 m_szUserMonitor[MAX_SIP_USER_MONITOR_LEN+1];
};
...
public:
    ...
    string GetSipWWWAuth() const;
    void SetSipWWWAuth(LPCSTR szSipAuInfo);
	...
	string GetSipNote() const;
	void SetSipNote(LPCSTR szSipNote);
    string GetUserMonitorId() const;
    void SetUserMonitorId(LPCSTR szUserMonitorId);
