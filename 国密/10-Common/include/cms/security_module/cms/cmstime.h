...
public:
	...
	inline CCMSTime( const xsd__dateTimer& t );
	inline CCMSTime( tm t );

	inline std::string ToString()const;		    /** 同 GetXSDDateTime */
	inline void FromString( const std::string& t );	// 同 SetXSDDateTime
	...
	inline void SetXSDDateTime( const xsd__dateTimer& t );
...
CCMSTime::CCMSTime( const xsd__dateTimer& t )
{
	this->SetXSDDateTime( t );
}
...
void CCMSTime::FromString( const std::string& t )
{
	SetXSDDateTime( t );
}
...
xsd__dateTimer CCMSTime::GetXSDDateTime() const
{
	...
	{
		sprintf_s( szTimezone, "+%02d:%02d", (int)abs(tz/3600), (int)(tz%3600) / 60 );
	}	
...
void CCMSTime::SetXSDDateTime( const xsd__dateTimer& xsdDataTime )
{
	tm tmTime;
	memset( &tmTime, 0, sizeof( tmTime ) );
	tm *a = &tmTime;
