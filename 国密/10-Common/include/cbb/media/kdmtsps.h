...
#define ERROR_PS_READ_OPUS_FRAME                TSPS_ERROR(404) //解码opus错误

#define  TSPS_MAX_VKEK_LEN            (32)    ///< VKEK字符串长度
#define  TSPS_MAX_VKEK_VERSION_LEN    (32)    ///< 描述信息最大长度
/************************************************************************/
/* PsTs接口                                                             */
/************************************************************************/
...
typedef enum tagTspsCipherMode
{
    TSPS_KDC_EXT_ENCRYPT,
    TSPS_KDC_EXT_DECRYPT,
    TSPS_KDC_EXT_INVALID,
} ETspsCipherMode;

typedef struct tagTspsMediaCipherParam
{
    ETspsCipherMode m_eMode;
    u32			m_dwEngineIdMd;
    u32			m_dwEngineIdCipher;
    u32			m_dwEngineIdPkey;
    u32			m_dwEngineIdRand;
} TTspsMediaCipherParam;
/* VKEK结构体定义 */
typedef struct tagTspsVkekInfo
{
    u8  m_abyVkek[TSPS_MAX_VKEK_LEN+1];   //vkek明文/密文字符串
    u32 m_dwVkekLen;                    //m_abyVkek中有效字符串长度	
    u8  m_abyVesion[TSPS_MAX_VKEK_VERSION_LEN+1]; //vkek版本号字符串
    u32 m_dwVesionLen;                  //m_abyVesion中有效字符串长度
    TTspsMediaCipherParam m_tCipherParam; //解密参数
} TspsMSVkekInfo;
...
/** 多路音视频扩展接口说明
*   TspsWriteSetProgramEx 为 TspsWriteSetProgram扩展，原有接口只支持单路音频和单路视频的设置，
*   新增TspsWriteSetProgramEx接口扩展为支持多路音频及视频
*   hWrite ：TspsWriteOpen创建的写对象
*   u8VideoSize：视频数量
*   pu8VideoTypeArray：视频类型队列
*	u8AudioSize：音频数量
*   pu8AudioTypeArray：音频类型队列
*/
u16 TspsWriteSetProgramEx(HTspsWrite hWrite, u8 u8VideoSize, u8* pu8VideoTypeArray, u8 u8AudioSize, u8* pu8AudioTypeArray);
...
u16 TspsReadInputStreamEx(HTspsRead hRead, u8 *pu8Buf, u32 u32Len, u32 dwRTPTime);
u16 TspsReadResetStream(HTspsRead hRead);
u16 TspsReadResetStreamExt(HTspsRead* phRead);
//设置tsps的vkek
u16 TspsReadSetVkek(HTspsRead hRead, const TspsMSVkekInfo* ptVkek);
u16 TspsReadCloseExt(HTspsRead hRead, BOOL32 bCloseCipher);
#ifdef __cplusplus
}
#endif  // __cplusplus
