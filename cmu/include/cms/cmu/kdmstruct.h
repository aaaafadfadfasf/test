...
-----------------------------------------------------------------------------
   ...
   2006/09/07  1.0         万春雷        修改编号规则，在原有8层域的基础上，
                                         ...
                                         一起作为域节点编号
   2013/01/16				沈鑫		 TKDMNO增加9900为最顶级域
******************************************************************************/
...
public:
	...
	TKDMNO GetSuperTopCmuNO(void) const; //得到最顶级[域/CMU平台]的顶级[域/CMU平台]编号，即是CMU的顶级CMU
	...
	//获取编号字串
	const s8* GetNOStr() const{ return m_achNO; };
	//设置编号字串
...
#define SUPER_TOP_KDM_NO (LPCSTR)"99000000000000000050000000000000"
#define INVALID_KDM_DOMAIN_LEVEL (s32)-1
...
{
	return SetNOString(tKdmNo.GetNOStr());
}
...
{
	s32 nLevel = INVALID_KDM_DOMAIN_LEVEL; //无效的Level
	if (NOFIELD_TYPE_INVALID != GetNOType())
	{
		...
		if (nLevel == 1 && GetCmuNO() == GetSuperTopCmuNO() )
		{
			return 0; //最顶级
		}
	}
...
// 	}
	...
	{
		return GetSuperTopCmuNO();
	}
	return tTopCmuId;
}

INLINEFLAG TKDMNO TKDMNO::GetSuperTopCmuNO(void) const
{
	TKDMNO tTopCmuId;
	tTopCmuId.SetNOString(SUPER_TOP_KDM_NO);
...
INLINEFLAG TKDMNO TKDMNO::GetNextCmu(const TKDMNO &tDstCMUId) const
{
    if(GetTradeNO() == tDstCMUId.GetTradeNO())
    {
        ...
        s32 nLevel    = GetDomainLevel();
		if ( INVALID_KDM_DOMAIN_LEVEL == nDstLevel || INVALID_KDM_DOMAIN_LEVEL == nLevel )
		{
			return TKDMNO();
		}
        ...
        else
        {
            ...
            {
				//如果目的是下级域
                if( 0 == nLevel || memcmp(m_achNO,tDstCMUId.m_achNO,nLevel*FIELD_LEN_SINGLE_DOMAIN)==0 ) 
                {
                    TKDMNO tMCmuId;
                    tMCmuId=tDstCMUId;
                    if (NOFIELD_TYPE_INVALID != GetNOType())
...
{    
	...
	//有其中有一个是9900，意味着相同的Cmu就是9900
	if ( 0 == sdwLevel || INVALID_KDM_DOMAIN_LEVEL == sdwLevel)
	{
		return GetSuperTopCmuNO();
	}
...
INLINEFLAG BOOL32 TKDMNO::IsCmuDown(const TKDMNO &tDownCmuId) const
{
	...
	if (sdwLocalLevel == 0 && sdwDstLevel != 0 )
	{
		return TRUE;
	}
...
INLINEFLAG s32 TKDMNO::GetSN( void ) const
{
	...
	case NOFIELD_TYPE_DOMAINORCMU://所在域的序号
		{
			s32 nLevel = GetDomainLevel();
			if (nLevel != INVALID_KDM_DOMAIN_LEVEL )
			{
				//如果是9900域，特殊处理
				if ( 0 == nLevel)
				{
...
INLINEFLAG u8 TKDMNO::GetDeviceGlobalType( void ) const
{
	...
	if (NOFIELD_TYPE_PLATFORMDEVICE == GetNOType())   //平台...
	{
		...
		case PLATFORM_DEVICE_TYPE_VPU:
			...
			break;
		case PLATFORM_DEVICE_TYPE_GAS:
			byGlobalDeviceType = DEVICE_GLOBAL_TYPE_GAS;
			break;
