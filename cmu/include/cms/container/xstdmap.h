...
public:
    ...
    {
    public:     
        explicit CStdIterator(ContainerBase& container,bool bEnd = false): m_pKey(0),m_pValue(0),
            m_bEnd(false), 
            m_container(container)
        {
        }
        CStdIterator(ContainerBase& container,CXIterator iter,bool bEnd = false):m_pKey(0),m_pValue(0),
        ...
        {           
            bool bGet = m_container.Iterate(*this,m_pKey,m_pValue);            
            if (!bGet)
            {
        ...
        bool operator==(const CStdIterator& iter)const
        {
            bool bEqual = (const CXIterator&)iter == &(const CXIterator&)(*this);
    ...
    public:
        ...
        VALUE* m_pValue;
        bool m_bEnd;//Iterate遍历时最后一个有效的item时CXIterator就已经到End了.
    protected:
...
public:
    ...
    {   
        ...
        bool bGet = ContainerBase::Iterate(iter.GetCXIter(),iter.m_pKey,iter.m_pValue); 
        if(!bGet)
        {
            return end();
        }
    ...
    iterator find(const KEY& theKey)
    {
		...
		{
			// go to next bucket
            for (size_t nBucket = pEntryRet->nHashValue + 1; nBucket < ContainerBase::m_nHashTableSize; nBucket++)
			{
