...
struct TGbsGbDomainChildConf
{
    ...
    string forbidCatalogChange;    // 是否禁止目录变更，"TRUE"--禁止，"FALSE"--不禁止；开启时相当于锁定下级目录，只处理下级设备上下线
...
public:
    ...
    void SetForbidCatalogChange(const string& strForbidCatalogChange)
    {
        forbidCatalogChange = strForbidCatalogChange;
    }
    string& GetForbidCatalogChange()
    {
        return forbidCatalogChange;
    }
    const string& GetForbidCatalogChange() const
    {
        return forbidCatalogChange;
    }
};
