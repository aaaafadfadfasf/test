...
typedef enum
{
	...
	RS_EVENT_DEL_DEV_NTF                        = RS_EVENT_BEGIN + 59,        /*TRS_OperResult*/
...
typedef struct tagRSDevDeadLine
{
	TRS_DevID           m_tDevID;         /*设备编号*/
	u32                 m_dwDeadLine;     /*设备录像时长，单位：天*/
}TRS_DevDeadLine;
...
typedef struct tagRSExtraChn
{
    u8                  m_byExtraChnNum;                            /*辅流数量*/
    s16                 m_wChannel[RS_MAX_EXTRA_CHN_NUM];           /*辅流通道*/
}TRS_ExtraChn;
...
typedef struct tagRSDeletePuBinding
{
    TRS_DevID          m_tDevID;            /*设备的id*/
    u16                m_wStartIndex;       /*起始索引*/
}TRS_DeletePuBinding;
...
typedef struct tagRSRecordParam
{
	...
	u32                     m_dwLockDeadLine;         /*录像保留时长，单位天*/
}TRS_RecordParam;
...
typedef struct tagRSRecQueryCond
{
    ...
    TRS_ExtraChn     m_tExtraChn;  // 辅流通道信息（主要应用于主辅流录像查询）
}TRS_RecQueryCond;
...
/*====================================================================
函数名      :RSDeletePuChnBinding
功能        :删除前端从某个通道开始以后的所有通道的绑定关系
算法实现    :
引用全局变量:
输入参数说明: const TRS_DeletePuBinding *ptDeletePuBinding  -- [IN] 需要清理绑定关系的前端信息
		      TRS_Sequence *ptSequence                    -- [OUT] 操作序列号
...
日  期      版本        修改人        修改内容
====================================================================*/
ERS_ErrType RSDeletePuBinding(const TRS_DeletePuBinding *ptDeletePuBinding, TRS_Sequence *ptSequence);
...
/*====================================================================
函数名      :RSUpdateDeadLine
功能        :更新录像时长
算法实现    :
引用全局变量:
输入参数说明: const TRS_DevDeadLine *ptDevDeadLine  -- [IN]  设备时长参数 
			  TRS_Sequence *ptSequence              -- [OUT] 操作序列号
...
修改记录    :
日  期      版本        修改人        修改内容
2018/03/09	1.0			Royan    	  创建
====================================================================*/
ERS_ErrType RSUpdateDeadLine(const TRS_DevDeadLine *ptDevDeadLine, TRS_Sequence *ptSequence);
