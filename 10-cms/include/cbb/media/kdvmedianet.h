...
#ifdef WIN32
#ifdef __cplusplus
#define KDVMEDIANET_API               extern "C" __declspec(dllexport)
#else
#define KDVMEDIANET_API               __declspec(dllexport)
#endif //__cplusplus
#else
#ifdef __cplusplus
#define KDVMEDIANET_API               extern "C" DLL_PUBLIC
#else
#define KDVMEDIANET_API               DLL_PUBLIC
#endif //__cplusplus
#endif
...
#define  ERROR_DATA_OVERRUN					(ERROR_MEDIA_NET_BASE+22)   ///<数据越界
...
#define  ERROR_RCV_INVALID_SOCK             (ERROR_MEDIA_NET_BASE+108)  ///<tcp接收模块无效socket
#define  ERROR_RCV_CONNECTTIMEOUT           (ERROR_MEDIA_NET_BASE+109)  ///<tcp接收模块connect超时
#define  ERROR_RCV_UNKNOW					(ERROR_MEDIA_NET_BASE+110)  ///<tcp接收模块未知错误
#define  ERROR_BINDSOCKET					(ERROR_MEDIA_NET_BASE+111)  ///<tcp绑定端口失败
#define  ERROR_SEMCREATE					(ERROR_MEDIA_NET_BASE+112)	///< 锁创建失败
#define  ERROR_TCP_INIT						(ERROR_MEDIA_NET_BASE+113)	///< 初始化TCP失败
#define  ERROR_CONNECTIONCLOSE              (ERROR_MEDIA_NET_BASE+114)	///< 对端关闭连接
#define  ERROR_NOINIT						(ERROR_MEDIA_NET_BASE+115)	///<没有初始化
#define  ERROR_AGAIN                        (ERROR_MEDIA_NET_BASE+115)	///< 需要重新发送
#define  ERROR_UNSUPPORT                    (ERROR_MEDIA_NET_BASE+116)	///< 不支持
#define  ERROR_OVER_PORTRANGE               (ERROR_MEDIA_NET_BASE+117)	///< 超过tcp端口范围
...
#define  ERROR_SND_H265FRAME                (ERROR_MEDIA_NET_BASE+220)  ///<解析H265失败
#define  ERROR_SND_H264FRAME                (ERROR_MEDIA_NET_BASE+221)  ///<解析H264失败
...
typedef struct tagFrameHdr
{
    ...
    u32    m_dwTimeStamp;   ///<时间戳, 用于接收端, 当ES流时与m_dwRtpTimeStamp保持一致，当PS或TS流时，则取用PES内携带的DTS时间戳
...
/**
 *@brief    设置亲核性，即tcp接收线程绑定在哪个CPU核上
 *@return   参见错误码定义
 *@ref
 *@see
 *@note    nCPUNum为CPU核数，CPU核数从0开始，例双核CPU即CPU核0，核1；超出实际CPU核数则不设置绑定
 *@note    该接口必须在KdvSocketStartup之前调用，该接口windows下无效
 */
u16 KdvSocketSetTCPThreadAffinity(s32 nCPUNum);
...
#define PS_MAX_VIDEO_ARRAY_SIZE 4
#define PS_MAX_AUDIO_ARRAY_SIZE 4
typedef struct tagPSInfoEx 
{
	BOOL32    m_bIsUse;
    u8        m_byVideoArraySize;
	u8        m_byVideoTypeArray[PS_MAX_VIDEO_ARRAY_SIZE];
    u8        m_byAudioArraySize;
	u8        m_byAudioTypeArray[PS_MAX_AUDIO_ARRAY_SIZE];
}TPSInfoEx;
...
public:
    ...
    /**
     *@brief 设置时间戳是否连续输出
     *@param[in]    BOOL bIsContinue 是否连续 TURE---表示连续输出，重新计算u32的翻转计数 FALSE---表示直接输出时间戳
     *@return       参见错误码定义
     *@ref
     *@see
     *@note  由于在接收到的RTP时间戳接收的上线大小除以90后，得到的时间戳在RTP时间戳翻转时，
		     实际的毫秒时间戳未打到最大值，所以这里我们修改为重新计算时间戳，
			 为了避免部分使用者会对毫秒时间进行乘以90进行转发的情况（该情况下u32极大值乘以90会导致时间戳翻转），我们增加了以下接口进行协调，
			 当使用者需要重新计算时间戳时设置为TRUE，此时回调的时间戳将重新计算，避免时间戳过早的翻转。
										 设置为FALSE，此时回调的时间戳将使用RTP时间戳除以采样率的方式进行回调。
     */
     u16 SetCBTimeStampCalc(BOOL32 bIsReCalc);
...
#endif
/** 网络事件类型 */
typedef enum tagTCPEventType
{
	/** 错误事件回调 */
	E_MT_TcpDisconnect = 100,	///< tcp断链(例如：异常断开，超过m_dwKeepliveTime没有收到数据)
	E_MT_MallocFailed  = 101,	///< 内存分配失败
	E_MT_TcpAcceptFail = 102,   ///< tcp accept失败
	E_MT_TcpDataError = 103,    ///< rtp数据错误

	/** 正确接收对方连接，回调fd/ip/port 等信息 */
	E_MT_TcpAccept     = 150,   ///<tcp accept 成功
}ETCPEventType;

/** 传输类型 */
typedef enum tagTCPTransType
{
	E_MT_UnknowTransType = 0,///< 未知传输类型
	E_MT_Tcp,				///< TTCPTransParam tcp传输参数 2字节头
	E_MT_YushiGBTcp,		///< TTCPTransParam tcp传输参数 4字节头
}ETCPTransType;
...
/** 网络参数 */
typedef struct tagMNRtpSession
{
	TTCPNetAddr    m_tRTPAddr;  ///< RTP地址(网络序)
	u16           m_wRTPPort;  ///< RTP端口(本机序)
}TTcpRtpSession;

/** tcp传输参数 */
typedef struct tagMNTcpTransParam
{
	TTcpRtpSession      m_tLocalNet;        ///< 本地网络参数
	TTcpRtpSession      m_tRemoteNet;       ///< 表示远端ip和端口
	BOOL32              m_bListen;		   ///< 是否为监听端
	ETCPTransType       m_eTransType;		///< 传输类型
}TTCPTransParam;

/** tcp回调参数 */
typedef struct tagMNTcpCallbackParam
{
	TTcpRtpSession      m_tRemoteNet;        ///< 远端网络参数
	u32					m_dwFd;              ///< 用于接收数据的fd
}TTCPCallbackParam;


/**
*@brief rtp data callback 
*@param[out]    dwFd 通过fd判断是哪条链路
*@param[out]    dwChnID 区分音视频 （传输类型设置为E_MT_YushiGBTcp时，此参数有效）
*@param[out]    pRtpData 回调的rtp包
*@param[out]    dwLenRtp 回调的rtp包长度
*@param[out]    pvContext 回调上下文
*/
typedef u32 (*KDVTcpRtpDataCallBack)(u32 dwFd, u32 dwChnID, u8* const pRtpData, u32 dwLenRtp, void* pvContext);


/**
*@brief 消息回调
*@param[out]    ptTcpCallbackParam 返回的参数
*@param[out]    dwRetCode 返回的消息类型
*@param[out]    pvContext 上下文
*@note dwRetCode：回调的事件类型
*      E_MT_TcpAccept：从ptTcpCallbackParam可以获取用于接收的fd，对端的ip和端口
*      其他事件类型：通过fd判断哪一路出现问题
*/
typedef u32 (*KDVTcpRtpMessageCallBack)(TTCPCallbackParam* ptTcpCallbackParam, u32 dwRetCode, void* pvContext);
...
/**
*@brief 设置全局tcp端口范围
*@param[in]    wStartPort   起始端口
*@param[in]    wEndPort   结束端口
*@return       参见错误码定义
*@note
*/
u16 KdvTcpPortRange(u16 wStartPort, u16 wEndPort);

/**
*@brief 创建inid，设置消息回调和数据回调
*@param[in]    pfKdvTcpRtpCallBackProc 数据回调
*@param[in]    pfKdvTcpRtpMessageCallBackProc 消息回调
*@param[in]    pvContext 上下文
*@param[out]   pdwId 创建的inid
*@return       参见错误码定义
*@note passive 模式一个id可能会维护多个fd，通过KdvTcpRemoveFd 可以移除其中一路
*      active  模式一个id对应一个fd 
*/
u16 KdvTcpIdCreate(KDVTcpRtpDataCallBack pfKdvTcpRtpCallBackProc, KDVTcpRtpMessageCallBack pfKdvTcpRtpMessageCallBackProc ,void* pvContext, u32* pdwId);


/**
*@brief 在端口范围内找到可以绑定的端口
*@param[in]    dwId   
*@param[in]    TNetAddr* ptLocalAddr 
*@param[out]   pwBindPort   返回值为成功的时候，此参数表示成功绑定的端口号
*@return       参见错误码定义
*@note
*/
u16 KdvTcpBindRange(u32 dwId, TTCPNetAddr* ptLocalAddr, u16* pwBindPort);

/**
*@brief 在端口范围内找到可以绑定的端口
*@param[in]    dwId   
*@param[in]    TNetAddr* ptLocalAddr 
*@param[in]   wBindPort   返回值为成功的时候，此参数表示成功绑定的端口号
*@return       参见错误码定义
*@note
*/
u16 KdvTcpBind(u32 dwId, TTCPNetAddr* ptLocalAddr, u16 wBindPort);

/**
*@brief 设置网络参数
*@param[in]    dwId   
*@param[in]    ptTransParam   需要设置的网络参数
*@return       参见错误码定义
*@note
*/
u16 KdvTcpIdSetParam(u32 dwId, TTCPTransParam* ptTransParam);

/**
*@brief 移除fd
*@param[in]    dwId
*@param[in]    sFd   需要移除的fd
*@return       参见错误码定义
*@note
*/
u16 KdvTcpRemoveFd(u32 dwId, s32 sFd);

/**
*@brief 释放inid，会同时移除inid对应的所有没有移除的fd
*@param[in]    dwId
*@return       参见错误码定义
*@note
*/
u16 KdvTcpIdRelease(u32 dwId);

u16 KdvTcpPauseReceive(u32 dwId, s32 nConnectFd);

u16 KdvTcpResumeReceive(u32 dwId, s32 nConnectFd);
