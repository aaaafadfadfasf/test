...
#define RSCOMM_MAXLEN_STORAGEVENDER     10
#define RSCOMM_STORAGEVENDER_KDM	    "kdm"
/************************************************************************/
/* 无效值                                                               */
/************************************************************************/
...
typedef enum
{
	RSCOMM_LOCKMODE_PUCHN = 1,
	RSCOMM_LOCKMODE_FILE,
}ERSComm_LockMode;
...
typedef enum{
	NRU_REC_STATUS_STOP,///default
	NRU_REC_STATUS_TRYING,
	NRU_REC_STATUS_START,
}ENruRecStatus;
...
public:
	...
	BOOL32 operator == ( const TRSComm_DevID &tDevID ) const
	{
		...
		if(0 == strcmp(m_szUUID, tDevID.m_szUUID))
		{
			return TRUE;
		}
		return FALSE;
	...
	BOOL32 operator > ( const TRSComm_DevID &tDevID ) const
	{
		if ( strcmp(m_szUUID, tDevID.m_szUUID) > 0)
		{
			return TRUE;
		}
...
public:
	...
	BOOL32 operator > (const TRSComm_DevChn& tDevChan) const
	{
		if (memcmp(this, &tDevChan, sizeof(TRSComm_DevChn)) > 0)
		{
			return TRUE;
		}
...
public:
	TRSComm_RecSysParam()
	{
		...
		m_dwDuration = 1800;//min is 300 for stop, 1800 for cover
	}
	void Clear()
	{
	...
	void SetCoverPolicy(const u8 byCoverPolicy)
	{
		...
		case RSCOMM_COVERPOLICY_COVERNORMAL:
			...
			break;
		default:
			m_eSysCoverPolicy = RSCOMM_COVERPOLICY_UNKNOWN;
			break;
		}
	}

	void SetDuration(const u32 dwVal){m_dwDuration = dwVal;}
	u32 GetDuration(){ return m_dwDuration;}

	void SetPermil(const u32 dwVal){m_wReservePermil = dwVal;}
	u32 GetPermil(){ return m_wReservePermil;}
	
protected:
	ERSComm_CoverPolicy   m_eSysCoverPolicy;
	u32  m_dwDuration; //seconds
	u16  m_wReservePermil; //千分比
}
#ifndef WIN32
__attribute__ ((packed))
#endif
...
public:
	...
	TRSComm_VideoEncParam& GetVideoEncParam()
	{
		return m_tVideoEncParam;
	}
...
public:
	...
	const TRSComm_IPAddress& GetNruAddr() const
	{
		return m_tNruAddr;
	}
...
public:
	...
	const ERSComm_MediaType GetMediaType() const
	{
		return m_eMediaType;
	};
...
public:
	...
	const ERSComm_TransportProtocol GetTransportProtocol() const
	{
		return m_eTransportProtocol;
	}
	...
	const ERSComm_StreamType GetStreamType() const
	{
		return m_eStreamType;
	}
...
public:
	...
	BOOL32 IsQueryByChnRange()
	{
		return (m_wQueryCond & (0x01 << 1));
	}

	BOOL32 IsQueryByTimeRangeStrictly()
	{
		return (m_wQueryCond & (0x01 << 2));
	}
	...
	void QueryByChnRange(BOOL32 bQueryByChn)
	{
		if (bQueryByChn)
		{
			m_wQueryCond |= (0x01 << 1);
	...
	void QueryByTimeRangeStrictly(BOOL32 bQueryByTimeRange)
	{
		if (bQueryByTimeRange)
		{
			m_wQueryCond |= (0x01 << 2);
...
public:
	...
	void SetPuChn(TRSComm_DevChn& tPuChn)
	{
		m_tPuChn = tPuChn;
	}
	...
	const u16 GetValidDay() const
	{
		return m_wValidDay;
	}

	void SetValidDay(const u16 wValidDay)
	{
		m_wValidDay =  wValidDay;
	}
...
public:
	...
	TRSComm_DeleteFile(TRSComm_RecFileSchema & tRecSchema)
	{
		m_dwRecID = tRecSchema.GetRecID();
		strcpy(m_szFilePath,tRecSchema.GetFilePath());
		strcpy(m_szFileName,tRecSchema.GetFileName());	
	}
...
public:
	...
	{
		m_dwRecStatusNotifyIntval = 10;
		m_dwRecNoStreamCheckIntval = 30;
	}
...
public:
	...
	BOOL32 IsRecLocked()
	{
		if (m_eRecLockStat == RSCOMM_REC_STAT_LOCK)
		{
			return TRUE;
		}
...
protected:
	...
	TRSComm_TimeRange     m_tTimeRange;                            /*录像时间段*/
	u32                 m_dwRecSize;
	ERSComm_RecLockStat	m_eRecLockStat;						 /*是否锁定*/
	u16                 m_wValidDay;                           /*锁定有效期，只有针对文件锁定时有效*/
}
#ifndef WIN32
__attribute__ ((packed))
#endif
...
public:
	...
	const u16  GetRecNum()const
	{
		return m_wRecNum;
	}
	void SetRecNum(const u16 wNum)
	{
		m_wRecNum = wNum;
	}
	...
	const u16  GetDownloadNum()const
	{
		return m_wDownloadNum;
	}
	...
	u32  GetMaxPlayNum()
	{
		return m_dwMaxPlayNum;
	}

	const u32  GetMaxPlayNum()const
	{
		return m_dwMaxPlayNum;
	}
	void SetMaxPlayNum(const u32 wMaxNum)
	{
		m_dwMaxPlayNum = wMaxNum;
	}

	u32  GetMaxDownNum()
	{
		return m_dwMaxDownloadNum;
	}

	const u32  GetMaxDownNum()const
	{
		return m_dwMaxDownloadNum;
	}
	void SetMaxDownNum(const u32 wMaxNum)
	{
		m_dwMaxDownloadNum = wMaxNum;
	}

	u32  GetMaxPreRecBw()
	{
		return m_dwMaxPreRecBandwidth;
	}

	const u32  GetMaxPreRecBw()const
	{
		return m_dwMaxPreRecBandwidth;
	}
	void SetMaxPreRecBw(const u32 wMaxBw)
	{
		m_dwMaxPreRecBandwidth = wMaxBw;
	}

	u32  GetMaxRecBw()
	{
		return m_dwMaxRecBandwidth;
	}

	const u32  GetMaxRecBw()const
	{
		return m_dwMaxRecBandwidth;
	}
	void SetMaxRecBw(const u32 wMaxBw)
	{
		m_dwMaxRecBandwidth = wMaxBw;
	}

	u32  GetMaxRecNum()
	{
		return m_dwMaxRecNum;
	}

	const u32  GetMaxRecNum()const
	{
		return m_dwMaxRecNum;
	}
	void SetMaxRecNum(const u32 wMaxNum)
	{
		m_dwMaxRecNum = wMaxNum;
	}
...
protected:
	...
	u32 		m_dwMaxPlayNum;     /*max player task number*/
	u32 		m_dwMaxDownloadNum;       /*max download task number*/
	u32  		m_dwMaxPreRecBandwidth;
	u32  		m_dwMaxRecBandwidth;//kbps
	u32  		m_dwMaxRecNum;
...
public:
	...
	const s8 * GetStorageVender() const 
	{
		return m_szStorageVender;
	}
	...
	void SetStorageVender(const s8* pszStorageVender)
	{
		if (NULL == pszStorageVender)
		{
			return;
		}

		s16 wCopyLen = strlen(pszStorageVender);
		wCopyLen = wCopyLen <= RSCOMM_MAXLEN_STORAGEVENDER ? wCopyLen : RSCOMM_MAXLEN_STORAGEVENDER;
		strncpy(m_szStorageVender, pszStorageVender, wCopyLen);
		m_szStorageVender[wCopyLen] = '\0';
	}

	void SetPartitionRecBW(u32 dwRecBw){m_dwRecBW = dwRecBw;}

	u32 GetPartitionRecBW(){return m_dwRecBW;}

    void SetPartitionRemainRecTime(u32 dwRemainRecTime){m_dwRemainRecTime = dwRemainRecTime;}

    u32 GetPartitionRemainRecTime(){return m_dwRemainRecTime;}
	...
	void PrintData(s8 * pszPrefix)
	{
		...
		OspPrintf(TRUE, FALSE, "%sStorageVender: %s\n", pszPrefix, m_szStorageVender);
		OspPrintf(TRUE, FALSE, "%sRecBW: %u kbps.\n", pszPrefix, m_dwRecBW);	
        OspPrintf(TRUE, FALSE, "%sRemainRecTime: %u s.\n", pszPrefix, m_dwRemainRecTime);
	}
...
protected:
	...
	s8					 m_szStorageVender[RSCOMM_MAXLEN_STORAGEVENDER+1]; /*存储设备厂商*/ //add by zhichun 20160825
	u32 				 m_dwRecBW;    /*录像带宽, kbps*/
    u32 				 m_dwRemainRecTime;    /*剩余录像时间, 秒*/
...
public:
	...
	const ERSComm_RunMode GetNruRunMode()const
	{
		return m_eNruRunMode;
	}
	...
	const TRSComm_MediaStreamInfo& GetMediaStreamCap()const
	{
		return m_tMediaStreamCap;
	}
	...
	const TRSComm_Time& GetSysTime() const
	{
		return m_tSysTime;
	}
	...
	const TRSComm_NetWork& GetNruNetWork() const
	{
		return m_tNruIpAddr;
	}
...
protected:
	...
	TRSComm_MediaStreamInfo       m_tMediaStreamCap;///nru 能力配置，nru配置文件中设置
...
typedef struct TDevInformation
{
	TDevInformation()
	{
		memset(this, 0, sizeof(TDevInformation));
	}

	s8  szUuID[RSCOMM_MAXLEN_UUID + 1];    /* uuid */
}
#ifndef WIN32
__attribute__ ((packed))
#endif
