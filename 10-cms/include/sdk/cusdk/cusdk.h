...
enum cusdk__ESubscribeNotifyType
{
	...
	cusdk__SubscribeDeviceGroupStat,			// 设备组统计信息(非递归). 按视频源统计
	cusdk__SubscribeDeviceGroupRecursiveStat,	// 设备组统计(递归设备组)信息. 按视频源统计

	cusdk__SubscribeDeviceGroupStatDevice,			// 设备组统计信息(非递归). 按设备统计
	cusdk__SubscribeDeviceGroupRecursiveStatDevice,	// 设备组统计(递归设备组)信息. 按设备统计
};
...
//设备ONOFF状态只有两种：OFF & ON
enum EPlatformDeviceOnoffStatus
{
	PLATFORM_DEVICE_STATUS_OFF = 0, //离线
	PLATFORM_DEVICE_STATUS_ON  = 1 //在线
};

/* 设备扩展能力 */
//扩展能力是s32类型（0x7FFFFFFF），低端值用于前端扩展能力集（0x00000000～0x3FFFFFFF），高端值用于平台扩展（0x40000000～0x7FFFFFFF）
const int DEVICE_REAL_ONOFF_STATUS = 0x40000001;       //KEY, VALUE参见EPlatformDeviceOnoffStatus
...
class cusdk__DevOnlineNotify
{
    ...
    xsd__boolean online;
	xsd__boolean deviceDisabled;	// 设备停用.
};
...
class cusdk__LoginReq
{
	...
	xsd__string clientVersion;	//cu客户端的版本号和cmsver一致
...
class cusdk__GetUserDevByGroupReq : public cusdk__BaseReq
{
	...
	xsd__string orderByColName;	// 设备排序字段名. 'devSeq'表示按序号排序. 'devName'表示按设备名称(GB2312拼音顺序)排序.
...
class cusdk__DeviceGroupAddGroupReq : public cusdk__BaseReq
{
	...
	bool filterDisabled;	// 是否过滤停用设备. +by lzx@2018-09-12 V2R3B3SP3
...
int cusdk__UpdateOperationProgress( cusdk__UpdateOperationProgreeReq req, cusdk__UpdateOperationProgreeRsp& rsp );
 ...
 // parentgroup		父组,不递归.
 // statVS=yes/no		是否统计设备总数和在线监控点数目. 默认不统计. 启用统计功能, 则 GetSearchResultByGroup 可以获取到监控点总数和在线数.
 // statDev=yes/no		是否统计设备总数和在线设备数目. 默认不统计. 启用统计功能, 则 GetSearchResultByGroup 可以获取到设备总数和在线数.
 // enabled=yes/no	是否筛选启用或停用的设备. +by lzx@2018-09-12 V2R3B3SP3
 // sortby=devSeq/devName 结果的排序方式,默认按设备序号排序. +by lzx@2019-01-16 V2R3B3SP3
 ...
 class cusdk__SearchResultItem
 {
	 ...
	 xsd__boolean disabled;	// 是否停用. +by lzx@2018-09-04 V2R3B3SP3
 };
 ...
 class cusdk__GetSearchResultByGroupRsp
 {
	 ...
	 xsd__int disabledVideoSrcNum;	// 停用视频源数目. statVS=yes 时启用此字段, 否则返回0. +by lzx@2018-09-04 V2R3B3SP3
 };
