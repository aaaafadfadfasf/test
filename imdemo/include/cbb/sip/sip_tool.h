...
/*国家标准GB/T 28181‐2011《安全防范视频监控联网系统信息传输交换控制技术要求》
  修改补充文件 2014*/
...
//证书校验摘要
#define CERTIFICATE_CHECK_ABSTRACT        "fingerprint"
...
typedef enum
{
    ...
    GB_MANSCDP_PTZCMD_TYPE_ASC,     ///6    ///辅助开关控制
...
typedef enum
{
    GB_MANSCDP_PTZCMD_TYPE_ASC_ON       = 1,    /// 辅助开关开
    GB_MANSCDP_PTZCMD_TYPE_ASC_OFF,    /// 2   /// 辅助开关关
}EGB_MANSCDP_PTZCMD_TYPE_ASC;
...
typedef struct
{
    u8                              byAuxiliarySwitchNo;   /// 辅助开关编号 0-255 1表示雨刷
    EGB_MANSCDP_PTZCMD_TYPE_ASC     eAction;
}TGB_MANSCDP_PTZCMD_TYPE_ASC;
...
typedef struct
{
    ...
    s32                 nRTPMapListSize;
    //GB28181 倍速参数 用于文件下载时控制下载速度 取值为整形 0值无效
    u32                 dwDownloadSpeed;
    //GB28181 文件大小 用于文件下载时进度计算 单位byte 0值无效
    u32                 dwFileSize;
    //证书校验摘要字段
    LPSTR               szCheckAbstract;
}TGB_SDP_MEDIA;
...
typedef enum
{
	...
	GB_SDP_MEDIA_VIDEO_ENCODETYPE_H265, /// 5
...
typedef enum
{
	...
	GB_SDP_MEDIA_ADUIO_ENCODETYPE_AACLC      = 11,   ///AAC-LC
	GB_SDP_MEDIA_ADUIO_ENCODETYPE_ADPCM,    ///12    ///ADPCM
	GB_SDP_MEDIA_ADUIO_ENCODETYPE_OPUS,     ///13    ///OPUS
...
typedef enum
{
	...
	GB_SDP_MEDIA_AUDIO_BITRATE_20Kbps             =21,    ///20 kbps
	GB_SDP_MEDIA_AUDIO_BITRATE_128Kbps,			          ///128 kbps
...
typedef enum
{
	...
	GB_SDP_MEDIA_AUDIO_SAMPLERATE_11_44_1kHz =11, /// 11   /// 11-44.1 kHz
	GB_SDP_MEDIA_AUDIO_SAMPLERATE_12_48kHz,       /// 12-48 kHz
...
{
    /*
      o=xx 0 0 IN IP4 192.168.0.101对应关系:
      o=szUsername_o session_id session_version szAddress_c
    */
    /*媒体流接收者设备编码 对应o=中的第一个字段*/
    LPSTR                      szUsername_o;
    /*作为会话描述中o= 地址字段，也作为c=的地址字段*/
    LPSTR                      szAddress_c;
    /*会话类型 对应s= 例如s=play*/
    LPSTR                      szSession_s;
    ...
    /* u行应填写视音频文件的URI。该URI取值有两种方式：简捷方式和普通方式。
        简捷方式直接采用产生该历史媒体的媒体源（如某个摄像头）的设备ID
    （应符合6.1.2的规定）以及相关参数，参数用"："分隔；
...
//创建，释放内存池
KDSIP_API HTOOL_MEMLIST SIP_TOOL_CreateMemList();
KDSIP_API void SIP_TOOL_FreeMemList(HTOOL_MEMLIST hMemList);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_S2TGB_MANSCDP_PTZCMD(HTOOL_MEMLIST hMemList, LPCSTR szIn,
        TGB_MANSCDP_PTZCMD* pvOut);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_TGB_MANSCDP_PTZCMD2S(TGB_MANSCDP_PTZCMD* pvIn, LPSTR szOut,
        s32* pnLen);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_S2TDATETIME(LPCSTR szTime, TGB_DATETIME* ptDateTime);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_TDATETIME2S(TGB_DATETIME* ptIn, LPSTR szOut, s32* pnLen,
        BOOL32 bUseMillisecond);//True:转换出带毫秒，FALSE:转换后不带毫秒

//notice: make sure initial ptOut, memsetting it with zero is suggested.
//上层业务务必保证将ptOut初始化为0
KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_S2TGB_SDP(HTOOL_MEMLIST hMemList, LPCSTR szIn, TGB_SDP* ptOut);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_TGB_SDP2S(TGB_SDP* ptIn,  LPSTR szOut, s32* pnLen);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_S2TGB_SUBJECT(HTOOL_MEMLIST hMemList, LPCSTR szIn,
        TGB_SUBJECT* ptOut);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_TGB_SUBJECT2S(TGB_SUBJECT* ptIn,  LPSTR szOut, s32* pnLen);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_S2TMANSRTSP(HTOOL_MEMLIST hMemList, LPCSTR szIn,
        EGB_MANSRTSP_METHOD* peMethod, void** ppvOut);

KDSIP_API ESIP_TOOL_RESULT
SIP_TOOL_CONVERT_TMANSRTSP2S(EGB_MANSRTSP_METHOD eMethod, void* ptIn,
        LPSTR szOut, s32* pnLen);
