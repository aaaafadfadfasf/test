...
enum eWorkType
{
	...
	emStreamVerifyFailed=30,	//码流验签失败，注意这个错误码是针对每帧的，可能会连续出现。该通知的m_dwErrorCode参数目前无意义（保留0)
	emStreamNoVKEK=31,	//本路浏览（TCP）没有VKEK，也就是本路不是安全前端的码流。 m_dwWorkID 表示播放ID,其他参数保留	，暂无意义
...
typedef struct tagMcusdkRecordPlan
{
	...
	bool bSupportChangeStreamType;       // 支持切换录像码流类型
	int nRecStreamType;                  // 录像码流类型。0-主流，1-辅流1，2-辅流2，3-辅流3

	bool bSupportSetRecDuration; //是否支持设置录像存储时长	
	bool bRecDurationEnabled; //当前是否启用录像存储时长设置
...
{
	char pemFile[260];//utf8表示的文件名 .pem 文件	，目前保留
	char keyFile[260];//utf8表示的文件名，.crt 文件，目前保留
	char keyPwd[MAX_PWD];//keyFile的文件读取保护密码，目前保留
	BOOL32 bSafeStream;//是否启用加密码流
}McusdkSafeModeInfo;
