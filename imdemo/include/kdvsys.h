...
/* SNMP拷贝FTP文件分割符 */
#define CHAR_FTPFILE_SEPARATOR		(s8)'|'	/* 不同文件之间分隔符 */
#define CHAR_FTPFILETYPE_SEPARATOR	(s8)':'	/* 文件名和类型之间分隔符 */
#define CHAR_EXE_TYPE				(s8)'E'	/* 二进制文件类型 */
#define CHAR_CFG_TYPE				(s8)'C'	/* 配置文件类型 */
#define CHAR_RES_TYPE				(s8)'R'   /* 资源文件类型 */
#define CHAR_WEB_TYPE				(s8)'W'	/* web文件类型 */
#define CHAR_PIC_TYPE				(s8)'P'	/* 图片文件类型 */
...
#define KDVSYS_VERSION "4.0"
DllExport const s8* GetKdvsysVersion();
...
DllExport BOOL32 GetRegKeyStringTable( const s8* lpszProfileName,    /* 文件名（含绝对路径）*/
                   ...
                   size_t usBufSize              /* 返回字串的长度，如不够则截断，最
                                                后一个字节填上'\0'    */
                 );
    /* 从配置文件中读取一张表里的若干行数据
 */
...
DllExport BOOL32 GetRegKeyString2( FILE *stream,  /* 文件句柄 */
                      ...
                      const s8* lpszDefault,       /* 失败时返回的默认值*/
                      s8* lpszReturnValue,    /* 返回字串          */
                      size_t BufSize           /* 返回字串的长度，如不够则截断，最
                                                后一个字节填上'\0'    */
                    );
